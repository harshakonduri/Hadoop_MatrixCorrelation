...
#SBATCH --job-name=org_gudelly
#SBATCH --mail-user=sreehars@buffalo.edu
#SBATCH --output=Result24.out
#SBATCH --error=Result_word_count24.out
...
echo
btime = $(date + "%s")
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar zapak.jar CardDriver harsha24.txt output24 1000000 20
atime = $(date + "%s")
ctime = $($(atime) - $(btime))
echo ctime
echo "ls files in dfs"
