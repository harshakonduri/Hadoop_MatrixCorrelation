...
#SBATCH --output=Result21.out
#SBATCH --error=Result_word_count21.out
...
echo
btime = $(date + "%s")
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar zapak.jar CardDriver harsha21.txt output21 50000 20
atime = $(date + "%s")
ctime = $($(atime) - $(btime))
echo ctime
echo "ls files in dfs"
