...
#SBATCH --output=Result22.out
#SBATCH --error=Result_word_count22.out
...
echo
btime = $(date + "%s")
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar zapak.jar CardDriver harsha22.txt output22 100000 20
atime = $(date + "%s")
ctime = $($(atime) - $(btime))
echo ctime
echo "ls files in dfs"
