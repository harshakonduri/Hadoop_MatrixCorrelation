...
#SBATCH --output=Result23.out
#SBATCH --error=Result_word_count23.out
...
echo
btime = $(date + "%s")
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar zapak.jar CardDriver harsha23.txt output23 100000 50
atime = $(date + "%s")
ctime = $($(atime) - $(btime))
echo ctime
echo "ls files in dfs"
